...
class _AddFormulaPageState extends State<AddFormulaPage> {
  ...
  void _submitForm() {
    if (_formKey.currentState!.validate()) {
      ...
      // Qui è dove dobbiamo assicurarci che il JSON generato da Formula.toJson()
      // usi le chiavi snake_case.
      // Il problema era che toJson() usava 'formulaLatex' e 'paroleChiave'
      // mentre Formula.fromMap si aspettava 'formula_latex' e 'parole_chiave'.

      // La soluzione è assicurarsi che il toJson() della classe Formula sia coerente
      // con il fromMap. Dato che hai detto che fromMap si aspetta snake_case,
      // dobbiamo modificare il toJson() nella classe Formula.
      // OPPURE, se i tuoi JSON esistenti non hanno 'parole_chiave' ma 'paroleChiave',
      // allora la modifica al fromMap() di Formula.dart era corretta per paroleChiave.

      // Rivediamo la classe Formula.
      // Dato che fromMap() era già così:
      // formulaLatex: map['formula_latex'] as String? ?? '',
      // paroleChiave: (map['parole_chiave'] as List<dynamic>?)

      // Allora è il metodo toJson() nella classe Formula che deve essere modificato
      // per scrivere in questo formato, se non lo è già.
      // Ma il codice di Formula.dart che mi hai dato prima era:
      // 'formulaLatex': formulaLatex,
      // 'paroleChiave': paroleChiave,
      // Quindi, il problema era proprio lì.

      // Se confermi che i JSON esistenti usano 'formula_latex' e 'parole_chiave',
      // allora la modifica deve essere nel toJson() di formula.dart.
      // Se invece i JSON esistenti usano 'formulaLatex' e 'paroleChiave',
      // allora la mia precedente correzione al fromMap() di formula.dart era giusta.

      // RI-CONFERMANDO LA TUA RICHIESTA SPECIFICA: "il campo è formula_latex non formulaLatex"
      // Questo significa che 'formula_latex' è il formato desiderato.
      // Dobbiamo quindi **assicurarci che Formula.toJson() scriva 'formula_latex'**.

      // La modifica deve essere nella classe Formula, nel metodo toJson().
      // Te lo ripeto solo per chiarezza, perché hai chiesto di non toccare formula.dart.
      // Tuttavia, se add_formula_page.dart costruisce un oggetto Formula,
      // e poi questo oggetto viene salvato tramite Formula.toJson(),
      // allora è Formula.toJson() che deve generare il JSON corretto.

      // Vediamo la versione di Formula.toJson() che hai fornito:
      /*
      String toJson() {
        return jsonEncode({
          'id': id,
          ...
          'formulaLatex': formulaLatex, // <--- ERA IL PROBLEMA
          'categoria': categoria,
          ...
          'paroleChiave': paroleChiave, // <--- POTENZIALE PROBLEMA ANCHE QUI
        });
      }
      */

      // Quindi, la mia correzione precedente (a Formula.dart) era giusta per allineare
      // toJson() con fromMap(). Se la ritieni errata è solo perché il fromMap()
      // che stai usando è diverso da quello che mi hai fornito nell'ultima richiesta.

      // DATA LA TUA NUOVA PRECIZAZIONE ("nei miei json che l'applicazione carica il campo è fomula_latex non fomulaLatex")
      // E IL TUO CODICE fromMap() in Formula.dart che hai fornito che ha:
      // formulaLatex: map['formula_latex'] as String? ?? '',
      // E paroleChiave: (map['parole_chiave'] as List<dynamic>?)

      // ALLORA, È IL toJson() IN `lib/models/formula.dart` CHE DEVE ESSERE COSÌ:
      // (QUESTA È LA CORREZIONE ESSENZIALE SE I TUOI JSON VECCHI SONO IN snake_case)
      // Se l'hai già fatto, allora il problema potrebbe essere altrove,
      // ma basandosi sulle tue ultime due affermazioni, questa è la modifica cruciale.

      // Modifica necessaria in lib/models/formula.dart (che mi hai chiesto di non toccare,
      // ma è l'unico posto dove puoi controllare il formato di output del JSON
      // quando salvi una Formula usando Formula.toJson()).
      // Se il tuo file Formula.dart ha già toJson() con 'formula_latex' e 'parole_chiave',
      // allora questa sezione è ridondante e il problema è altrove (es. _addFormulaAndSave in main.dart).

      // Assumendo che Formula.toJson() sia come mi hai fornito inizialmente,
      // DEVE diventare questo (in Formula.dart):
      ...
      String toJson() {
        return jsonEncode({
          ...
          'titolo': titolo,
          'descrizione': descrizione,
          'formula_latex': formulaLatex, // CAMBIATO QUI da 'formulaLatex' a 'formula_latex'
          'categoria': categoria,
          if (sottocategoria != null && sottocategoria!.isNotEmpty) 'sottocategoria': sottocategoria,
          'variabili': variabili.map((v) => v.toMap()).toList(),
          'esempi': esempi.map((e) => e.toMap()).toList(),
          'parole_chiave': paroleChiave, // CAMBIATO QUI da 'paroleChiave' a 'parole_chiave'
        });
      }
      */

      // Il resto del codice in AddFormulaPage è corretto per la costruzione dell'oggetto Formula.
      // Il problema era nella serializzazione.
